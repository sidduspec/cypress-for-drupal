...
When('I add a document with the name {string} and file {string}', (mediaName, mediaFilePath) => {
...
Then('I update the {string} for {string} with {string} and assert its presense in the media table', (mediaType, mediaName, mediaFile) => {
  cy.updateMediaAndAssert(mediaType, mediaName, mediaFile);
})

Then('I update the {string} for {string} with {string} and alt text {string} and assert its presense in the media table', (mediaType, mediaName, mediaFile, altText) => {
  cy.updateMediaAndAssert(mediaType, mediaName, mediaFile, altText);
})

When('I add an image with the name {string}, file {string}, and alt text {string}', (mediaName, mediaFilePath, altText) => {
  cy.addImage(mediaName, mediaFilePath, altText); // Call the custom command to add an image
});

When('I add a remote video with the name {string} and URL {string}', (mediaName, videoURL) => {
...
When('I add and validate a media type {string} with description {string} and source {string}', (mediaTypeName, mediaDescription, mediaSource) => {
